...
for dir in folderNames:
    ...
    if filename != '':
        tmp = pd.read_excel(filename, sheet_name='учебники')
        
        column = tmp['Форма 5_2(0)'].tolist()
        ...
        for i in range(0,len(tableNums)):
            ...
            if i==(len(tableNums)-1):
                ...
                tmpDataFrames[i] = tmpDataFrames[i][:-4]
            else:
...
os.chdir(scriptdir)
excelData.to_excel("F5_2(0)_output.xlsx",index=False,header=False,sheet_name='учебники')  
