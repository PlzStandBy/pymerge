...
for dir in folderNames:
    ...
    if filename != '':
      ...
      if isFirstFile:
          tmp = tmp.drop(list(range(5)))
          isFirstFile = False
      else:
          #Удаляю первые ВМЕСТЕ С ШАПКОЙ
          tmp = tmp.drop(list(range(7))) 
      #Удаляю последние 
...
os.chdir(scriptdir)
excelData.to_excel("F4_output.xlsx",index=False,header=False,sheet_name='поданные проекты') 