...
for dir in folderNames:
    ...
    for name in fileNames:
        flag = False
        #Назвние форм, с которыми сейчас работаем
        matchPos = re.search('F4_1', name)
        names = sourceNames.copy()
        names.remove('F4_1')
        names.remove('F4')
    ...
    if filename != '':
      ...
      if isFirstFile:
          tmp = tmp.drop([0,1,2,3,4])
          isFirstFile = False
      else:
          #Удаляю первые ВМЕСТЕ С ШАПКОЙ
          tmp = tmp.drop([0,1,2,3,4,5,6]) 
      #Удаляю последние 
...
os.chdir(scriptdir)
excelData.to_excel("F4_1_output.xlsx",index=False,header=False,sheet_name='поданные проекты')  
